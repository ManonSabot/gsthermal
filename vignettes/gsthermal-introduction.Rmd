---
title: "Introduction to `gsthermal`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Introduction to `gsthermal`}
...
E = trans_from_vc(P, kmax_25, Tair, b, c)
...
$$T_{leaf} = T_{air} + \frac {R_{net} - \lambda E}{C_p (g_{Ha} + g_r)},$$ where $R_{net}(T_{air}, PPFD, VPD)$ is the net radiation (W m^-2^), $\lambda (T_{air})$ is the latent heat of vaporization (J mol^-1^), $C_p$ is the specific heat of air at constant pressure (J mol^-1^ $^\circ$C^-1^), $g_{Ha}$ is the boundary layer conductance to forced convection (mol m^-2^ s^-1^), and $g_r$ is the radiative conductance (mol m^-2^ s^-1^).

From $T_{leaf}$, we calculate $D_{leaf} = e_{sat}(T_{leaf}) - e_{sat}(T_{air}) - VPD$, $g_w = \frac {P_{atm} E} {D_{leaf}}$, and finally the photosynthetic rate $A$ from the **Photosyn** function in the package **plantecophys**.

```{r physio}
Tleaf = calc_Tleaf(Tair = Tair, E = E, VPD = VPD)
D_leaf = plantecophys::VPDairToLeaf(VPD = VPD, Tleaf = Tleaf, Tair = Tair)
g_w = calc_gw(E = E, Tleaf = Tleaf, Tair = Tair, VPD = VPD)
A = calc_A(Tair = Tair, E = E, VPD =VPD)

plot(P, Tleaf, type = "l", col = "black", xlab = "Leaf water potential (-MPa)",
...
```{r thermal damage}
Tleaf_for_F0vT = seq(30, 54, length.out = 400)
Tcrit = 48
T50 = 51
F0_min = 500
F0_max = 1000

F0_for_F0vT = F0_func(Tleaf_for_F0vT, Tcrit, T50, F0_max, F0_min)
plot(Tleaf_for_F0vT, F0_for_F0vT, type = 'l',
...
```{r cost-gain}
hydraulic_cst = hydraulic_cost(P = P, Tair = Tair, kmax_25 = kmax_25)
thermal_cst = thermal_cost(P = P, Tair = Tair, VPD = VPD, Tcrit = Tcrit, T50 = T50)
carbon_gain = C_gain(P = P, Tair = Tair, VPD = VPD, kmax_25 = kmax_25)
...
kmax_pl = calc_kmax(kmax_25 = 4, Tair = temp_range)
